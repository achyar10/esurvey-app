...
const Index = () => {
    ...
    const [data, setData] = useState<IJobStatus[]>([]);
    ...
    useEffect(() => {
        const fetch = async () => {
            setLoading(true);
            try {
                const response = await jobStatusService.get({ page, limit });
    ...
    const save = async (val: any) => {
        ...
        try {
            if (!val.id) {
                await jobStatusService.create({ name })
                toastUtil.useAlert('Tambah Data berhasil', 'success')
                setModal(false)
                setUpdate(true)
            } else {
                await jobStatusService.update({ id: val.id, name })
    ...
    return (
        <>
            <CRow>
                <CCol md={12}>
                    <div className="mb-3">
                        <h5>Daftar Status Pekerjaan <button className="btn btn-sm btn-danger text-white float-end" onClick={() => toggle()}><CIcon icon={cilPlus} /> Tambah Status Pekerjaan</button></h5>
                    </div>
            ...
            <CModal visible={modal} onClose={() => setModal(false)}>
                ...
                <CModalBody>
                    <div className="mb-3">
                        <CFormLabel htmlFor="name">Nama Status Pekerjaan <span className="text-danger">*</span></CFormLabel>
                        <CFormInput type="text" id="name" placeholder="Cth: PNS" onChange={(e) => setObj({ ...obj, name: e.target.value })} value={obj?.name || ''} />
                    </div>
                </CModalBody>
                <CModalFooter>
