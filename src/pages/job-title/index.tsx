...
const Index = () => {
    ...
    const [data, setData] = useState<IJobTitle[]>([]);
    ...
    useEffect(() => {
        const fetch = async () => {
            setLoading(true);
            try {
                const response = await jobTitleService.get({ page, limit });
    ...
    const save = async (val: any) => {
        ...
        try {
            if (!val.id) {
                ...
                await jobTitleService.update({ id: val.id, name })
    ...
    return (
        <>
            <CRow>
                <CCol md={12}>
                    <div className="mb-3">
                        <h5>Daftar Jabatan Pekerjaan <button className="btn btn-sm btn-danger text-white float-end" onClick={() => toggle()}><CIcon icon={cilPlus} /> Tambah Jabatan Pekerjaan</button></h5>
                    </div>
            ...
            <CModal visible={modal} onClose={() => setModal(false)}>
                ...
                <CModalBody>
                    <div className="mb-3">
                        <CFormLabel htmlFor="name">Nama Jabatan Pekerjaan <span className="text-danger">*</span></CFormLabel>
                        <CFormInput type="text" id="name" placeholder="Cth: Eselon 1" onChange={(e) => setObj({ ...obj, name: e.target.value })} value={obj?.name || ''} />
                    </div>
                </CModalBody>
                <CModalFooter>
