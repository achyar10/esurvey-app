...
const Index = () => {
    ...
    const [data, setData] = useState<IQuestionCategory[]>([]);
    ...
    useEffect(() => {
        const fetch = async () => {
            setLoading(true);
            try {
                const response = await questionCategoryService.get({ page, limit });
    ...
    const save = async (val: any) => {
        ...
        try {
            if (!val.id) {
                await questionCategoryService.create({ name, code })
                toastUtil.useAlert('Tambah Data berhasil', 'success')
                setModal(false)
                setUpdate(true)
            } else {
                await questionCategoryService.update({ id: val.id, name, code })
    ...
    return (
        <>
            <CRow>
                <CCol md={12}>
                    <div className="mb-3">
                        <h5>Daftar Unsur Pelayanan <button className="btn btn-sm btn-danger text-white float-end" onClick={() => toggle()}><CIcon icon={cilPlus} /> Tambah Unsur Pelayanan</button></h5>
                    </div>
            ...
            <CModal visible={modal} onClose={() => setModal(false)}>
                ...
                <CModalBody>
                    <div className="mb-3">
                        <CFormLabel htmlFor="code">Kode Unsur <span className="text-danger">*</span></CFormLabel>
                        <CFormInput type="text" id="code" placeholder="Cth: U1" onChange={(e) => setObj({ ...obj, code: e.target.value })} value={obj?.code || ''} />
                    </div>
                    <div className="mb-3">
                        <CFormLabel htmlFor="name">Nama Unsur <span className="text-danger">*</span></CFormLabel>
                        <CFormInput type="text" id="name" placeholder="Cth: Persyaratan" onChange={(e) => setObj({ ...obj, name: e.target.value })} value={obj?.name || ''} />
                    </div>
                </CModalBody>
                <CModalFooter>
